...
def analyze_video_motion(
    video_path: str, arrow_dir_x: float = 1.0, arrow_dir_y: float = 0.0
):
    """
    ...
    print(
        f"{'Frame':<8} {'Time(s)':<10} {'Flow_X':<12} {'Flow_Y':<12} {'Magnitude':<12}"
    )
    print("-" * 60)
    ...
    try:
        while True:
            ...
            if flow_x is not None and flow_y is not None:
                ...
                print(
                    f"{frame_count:<8} {current_time:<10.2f} {flow_x:<12.4f} {flow_y:<12.4f} {magnitude:<12.4f}"
                )
            else:
                print(
                    f"{frame_count:<8} {current_time:<10.2f} {'N/A':<12} {'N/A':<12} {'N/A':<12}"
                )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python video_motion_analyzer.py <video_file_path> [arrow_dir_x] [arrow_dir_y]"
        )
        print("\nExample:")
        print("  python video_motion_analyzer.py video.mp4")
        print(
            "  python video_motion_analyzer.py video.mp4 1.0 0.0  # Analyze rightward motion"
        )
        print(
            "  python video_motion_analyzer.py video.mp4 0.0 1.0  # Analyze downward motion"
        )
        sys.exit(1)
