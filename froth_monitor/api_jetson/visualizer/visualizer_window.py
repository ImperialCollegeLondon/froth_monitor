...
class VisualizerWindow(QtWidgets.QMainWindow):
    ...
    def add_distance(self, timestamp: str, distance_mm: float):
        """
        Call this whenever you have a new measurement.
        :param timestamp: timestamp string in format "%H:%M:%S.%f"
        :param distance_mm: measured distance in mm
        """
        # avoid None in distance
        if distance_mm is None:
            return
        
        now = time.time()
        
        # Convert string timestamp to absolute time for latency calculation
        try:
            # Parse timestamp string to datetime, then convert to epoch time
            today = datetime.now().date()
            timestamp_dt = datetime.strptime(timestamp, "%H:%M:%S.%f")
            # Combine with today's date to get full datetime
            timestamp_dt = timestamp_dt.replace(year=today.year, month=today.month, day=today.day)
            timestamp_epoch = timestamp_dt.timestamp()
            
            # latency in ms
            latency_ms = (now - timestamp_epoch) * 1000.0
            self.latency_label.setText(f"Latency: {latency_ms:.1f} ms")
            
            # plot data - use relative time from start
            rel = timestamp_epoch - self.start_time
