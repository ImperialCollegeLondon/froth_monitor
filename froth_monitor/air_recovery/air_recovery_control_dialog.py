...
class AirRecoveryControlDialog(QDialog):
        ...
        try:
            ...
            if self.jg_flow_radio.isChecked():
                self.air_recovery_processor.set_jg_parameters(
                    self.jg_spin.value(), self.cell_area_spin.value()
                )
            else:
                self.air_recovery_processor.set_air_flow_rate(
                    self.flow_rate_spin.value(), self.flow_unit_combo.currentText()
                )
            ...
            QMessageBox.warning(
                self, "Configuration Error", f"Failed to apply configuration: {str(e)}"
            )
    ...
    def reset_configuration(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                QMessageBox.warning(
                    self, "Reset Error", f"Failed to reset configuration: {str(e)}"
                )
    ...
    def clear_data(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            ...
            QMessageBox.information(
                self, "Data Cleared", "All air recovery data has been cleared."
            )
    ...
    def update_display(self):
        ...
        try:
            ...
            current_air_recovery = (
                self.air_recovery_processor.get_current_air_recovery()
            )
            self.air_recovery_label.setText(f"{current_air_recovery:.2f} %")

            self.velocity_label.setText(
                f"{self.air_recovery_processor.current_velocity:.1f} mm/s"
            )
            self.froth_height_label.setText(
                f"{self.air_recovery_processor.current_froth_height:.1f} mm"
            )
