...
from PySide6.QtWidgets import (QApplication, QFileDialog, QMessageBox, QDialog, QComboBox, QPushButton, QVBoxLayout, QMenu, QStyleFactory)
...
class EventHandler:
    ...
    def import_local_video(self):
        ...
        if file_path:
            ...
            if self.camera_thread.start_capture(file_path):
                ...
                self.frame_width, self.frame_height = self.camera_thread.get_frame_dimensions()
                
                # Start playing the video
                self.playing = True
                self.initialze_tool_window()
            else:
                QMessageBox.critical(self.gui, "Error", "Could not open the video file!")
                return
    ...
    def load_selected_camera(self, camera_combo, dialog):
        ...
        if self.camera_thread.start_capture(camera_index):
            ...
            QMessageBox.critical(self.gui, "Error", "Could not open the selected camera!")
            return
    ...
    def process_new_frame(self, frame):
        ...
        resized_frame = self._create_resized_frame(frame, scaled_image.width(), scaled_image.height())
    ...
    def _scale_image_to_canvas(self, qt_image):
        ...
        """
        return qt_image.scaled(self.canvas_width, self.canvas_height, Qt.AspectRatioMode.KeepAspectRatio)
    ...
    def _process_frame_with_model(self, resized_frame):
        ...
        """
        self.current_frame_number, roi_list = self.frame_model.process_frame(resized_frame)
    ...
    def _update_status_bar(self):
        ...
        if hasattr(self.gui, 'statusBar'):
            self.gui.statusBar().showMessage(f"Frame: {self.current_frame_number} | Time: {self.frame_model.last_processed_time}")
    ...
    def start_ruler_calibration(self):
        ...
        if self.confirm_calibration:
            QMessageBox.warning(self.gui, "Warning", "You have already confirmed the arrow and ruler. Please reset the application if you want to change them.")
            return
        # Start ruler calibration mode
        self.overlay_widget.ruler_calibration()
        
        # Inform the user
        self.gui.statusBar().showMessage("Click and drag to draw a line of 2cm for pixel measurement")
    ...
    def handle_ruler_measurement(self, px_distance):
        ...
        QMessageBox.information(self.gui, "Ruler Calibration", f"Px to mm ratio: {self.frame_model.px2mm:.1f} per mm")
            
        # Update the status bar
        self.gui.statusBar().showMessage(f"Px to mm ratio: {self.frame_model.px2mm:.1f} per mm")
    ...
    def add_roi(self):
        ...
        if not self.camera_thread.is_running():
            QMessageBox.warning(self.gui, "Warning", "No video source loaded! Please load a video first.")
            return
        ...
        # Inform the user
        self.gui.statusBar().showMessage("Click and drag to draw a Region of Interest rectangle")
    ...
    def handle_roi_created(self, rect):
        ...
        # Inform the user
        self.gui.statusBar().showMessage(f"ROI created at ({video_x}, {video_y}) with size {video_width}x{video_height}")
    ...
    def confirm_arrow_n_ruler(self):
        ...
        if self.frame_model.px2mm is None:
            QMessageBox.warning(self.gui, "Warning", "Please calibrate the ruler first.")
            return
        ...
        try:
            ...
            print(ValueError)
            QMessageBox.warning(self.gui, "Warning", "Please enter valid arrow direction and px2mm values.")
            return

        self.confirm_calibration = True
        QMessageBox.information(self.gui, "Info", "Overflow direction (arrow) and calibration (ruler) confirmed.")
    ...
    def handle_arrow_drawing(self, start_pos, end_pos, degree):
        ...
        # Display the measurement result to the user
        QMessageBox.information(self.gui, "Arrow drawed", f"angle: {degree:.1f} degrees (from the horizontal axis anticlockwisely)")
    ...
    def toggle_recording(self):
        ...
        QMessageBox.information(self.gui, "Info", "Recording functionality will be implemented.")
    ...
    def save_data(self):
        ...
        self.export.excel_results(self.frame_model.roi_list,
    ...
    def update_velocity_plot(self):
        ...
        colors = ['r', 'g', 'b', 'c', 'm', 'y', 'w']  # Red, green, blue, cyan, magenta, yellow, white
        ...
        max_velocity = 0
        if self.frame_model.roi_list and any(roi.velo_only_history for roi in self.frame_model.roi_list):
            max_velocity = max(max(roi.velo_only_history) if roi.velo_only_history else 0 
