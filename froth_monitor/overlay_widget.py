...
from PySide6.QtCore import Qt, QTimer, QSize, QRect, QPoint, Signal
...
class OverlayWidget(QWidget):
    ...
    arrow_drawn = Signal(QPoint, QPoint, float)  # Signal to emit start point, end point, and angle
    ...
    def paintEvent(self, event):
        ...
        if self.drawing_roi:
            ...
            if self.drawing_roi and not self.roi_start_point.isNull() and not self.roi_end_point.isNull():
                # Draw the rectangle with a semi-transparent fill
                roi_rect = QRect(self.roi_start_point, self.roi_end_point).normalized()
                painter.fillRect(roi_rect, QColor(0, 255, 0, 50))  # Green with 20% opacity
                # Draw the rectangle border
                painter.setPen(QPen(QColor(0, 255, 0, 200), 2))  # Green with 80% opacity, 2px width
                painter.drawRect(roi_rect)
        
        elif self.drawing_ruler:
            # Draw ruler line if we're in ruler calibration mode
            if not self.ruler_start_point.isNull() and not self.ruler_end_point.isNull():
                # Draw the line
                painter.setPen(QPen(QColor(255, 0, 0, 200), 2))  # Red with 80% opacity, 2px width
                ...
                if self.ruler_distance > 0:
                    ...
                    for dx in [-1, 0, 1]:
                        for dy in [-1, 0, 1]:
                            painter.drawText(text_x + dx, text_y + dy, f"{self.ruler_distance:.1f} px")
            ...
            if not self.arrow_start_point.isNull() and not self.arrow_end_point.isNull():
                # Draw the arrow line
                painter.setPen(QPen(QColor(255, 165, 0, 200), 2))  # Orange with 80% opacity, 2px width
                painter.drawLine(self.arrow_start_point, self.arrow_end_point)
                
                # Draw the arrowhead
                self.draw_arrowhead(painter, self.arrow_start_point, self.arrow_end_point)
                ...
                if self.arrow_start_point != self.arrow_end_point:
                    ...
                    for dx in [-1, 0, 1]:
                        for dy in [-1, 0, 1]:
                            painter.drawText(text_x + dx, text_y + dy, f"{self.arrow_angle:.1f}°")
                    painter.setPen(QColor(255, 255, 255))
    ...
    def mouseReleaseEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            if self.drawing_roi:
                ...
                self.current_roi_rect = QRect(self.roi_start_point, self.roi_end_point).normalized()
                ...
                self.arrow_drawn.emit(self.arrow_start_point, self.arrow_end_point, self.arrow_angle)
    ...
    def get_roi_coordinates(self):
        ...
        if self.current_roi_rect.isValid():
            return (self.current_roi_rect.x(), self.current_roi_rect.y(), 
                    self.current_roi_rect.width(), self.current_roi_rect.height())
        return None
    ...
    def drawROIs(self, painter):
        ...
        for i, roi in enumerate(self.roi_list):
            ...
            # Draw the rectangle border
            painter.setPen(QPen(QColor(0, 200, 255, 200), 2))  # Light blue with 80% opacity, 2px width
            ...
            if roi.delta_pixels is not None:
                ...
                painter.setPen(QPen(QColor(255, 50, 50, 230), 2))  # Red with 90% opacity, 2px width
