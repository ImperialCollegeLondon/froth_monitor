...
class VideoHandler(QObject):
    ...
    def load_jetson(self):
        # Connect the signal BEFORE starting network capture
        self.network_thread.connection_status_changed.connect(
            lambda connected, message: self.status_message.emit(message)
        )
        ...
        if self.network_thread.start_network_capture(self.jetson_source_address,
            ...
            self.frame_width, self.frame_height = (
                self.network_thread.get_frame_dimensions()
            )
            self.if_jetson = True
            self.playing = True
            self.playback_state_changed.emit(True)
            ...
            self.info_occurred.emit("Error", "Failed to start network capture.")
