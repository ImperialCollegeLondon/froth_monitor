...
class CalibrationHandler(QObject):
    ...
    def confirm_arrow_n_ruler(self):
        ...
        if self.confirm_calibration:
            message = (
                "You have already confirmed the arrow and ruler. "
                "Please reset the application if you want to change them."
            )
            QMessageBox.warning(
                self.gui,
                "Warning",
        ...
        try:
            ...
            logger.error(f"Error when confirming calibration: {ValueError}")
            QMessageBox.warning(
                self.gui,
                "Warning",
...
class DataHandler:
    ...
    def update_arec_data(self):
        ...
        logger.info("Updating air recovery data...")
...
class VelocityLidarMatcher(QObject):
    ...
    def _check_for_match(self):
        ...
        if len(current_lidar_history) > self.initial_lidar_count:
            ...
            logger.info(f"New lidar timestamp: {new_latest_timestamp}")
            logger.info(f"Target timestamp: {self.target_timestamp}")
...
class LidarHandler:
    ...
    def update_fh_plot(self, lidar_reading_history):
        self.velocity_plotter.update_fh_plot(lidar_reading_history)
